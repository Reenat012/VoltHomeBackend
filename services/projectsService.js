...
import { upsertRooms, deleteRooms, deltaRooms, getRoomsByProject } from "../models/rooms.js";
import { upsertGroups, deleteGroups, deltaGroups, getGroupsByProject } from "../models/groups.js";
import { upsertDevices, deleteDevices, deltaDevices, getDevicesByProject } from "../models/devices.js";
...
export async function getDelta({ userId, projectId, since }) {
    ...
    const rooms = { upsert: r.filter(x => !x.is_deleted), delete: r.filter(x => x.is_deleted).map(x => x.id) };
    const groups = { upsert: g.filter(x => !x.is_deleted), delete: g.filter(x => x.is_deleted).map(x => x.id) };
    const devices = { upsert: d.filter(x => !x.is_deleted), delete: d.filter(x => x.is_deleted).map(x => x.id) };
...
export async function applyBatch({ userId, projectId, baseVersion, ops }) {
    ...
    await query("BEGIN");
    try {
        // В LWW мы не сравниваем по полям; на уровне сущностей сервер берёт "последнюю запись".
        // В данном шаге LWW реализуем как: апсерты перезаписывают, delete ставит tombstone.
        if (ops?.rooms?.upsert?.length) await upsertRooms(projectId, userId, ops.rooms.upsert);
       ...
       VALUES ($1, 'batch', 'projects', $2::jsonb)`,
            [userId, JSON.stringify({ projectId, baseVersion, newVersion, counts: {
