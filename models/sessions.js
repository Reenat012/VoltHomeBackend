...
/**
 * ВАЖНО:
 * Раньше здесь возвращался Buffer от hex-строки, что приводило к вставке не-UTF8 байтов
 * в TEXT-колонку token_hash и ошибке вида "invalid byte sequence for encoding UTF8".
 * Теперь возвращаем ЧИСТУЮ HEX-СТРОКУ.
 */
...
export async function createSession({ userId, refreshToken, userAgent, ip, now = new Date() }) {
    ...
    const tokenHash = sha256Hex(refreshToken); // строка hex
...
export async function rotateSession({ oldSessionId, userId, newRefreshToken }) {
    const tokenHash = sha256Hex(newRefreshToken); // строка hex
    const res = await query(
        `WITH ins AS (
