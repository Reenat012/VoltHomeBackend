 ...
 * ВАЖНО: сигнатура (projectId, items) — без userId, чтобы совпадать с вызовами из services.
 */
export async function upsertRooms(projectId, items) {
    ...
    for (const r of items) {
        ...
        params.push(
            r.id || null,                                  // id (может прийти с клиента)
            projectId,                                     // project_id
            r.name ?? null,                                // name
            r.meta ? (typeof r.meta === "string" ? r.meta : JSON.stringify(r.meta)) : null // meta
        );
    }
    const sql = `
