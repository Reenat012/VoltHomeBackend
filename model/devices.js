...
export async function upsertDevices(projectId, items) {
    ...
    for (const d of items) {
        ...
        params.push(d.id || null, projectId, d.name, d.meta ? JSON.stringify(d.meta) : null);
    }
    const sql = `
    INSERT INTO devices(id, project_id, name, meta, updated_at, is_deleted)
    VALUES ${values.join(",")}
...
export async function deltaDevices(projectId, sinceIso) {
    const res = await query(
     ...
     FROM devices WHERE project_id=$1 AND updated_at > $2
